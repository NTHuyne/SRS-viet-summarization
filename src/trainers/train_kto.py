...
from configs.kto_config import get_kto_config
from configs.lora_config import get_lora_config
from utils.model_utils import load_model, load_tokenizer
from data.dataset_loader import load_kto_dataset


@dataclass
class ScriptArguments:
    """Arguments for KTO training script"""
    ...
    beta: float = field(default=0.1, metadata={"help": "KTO beta parameter"})
...
def main():
    ...
    print(f"Desirable Weight: {script_args.desirable_weight}")
    print(f"Undesirable Weight: {script_args.undesirable_weight}")
    print("="*50)
    ...
    # Load dataset
    print("[3/4] Loading KTO dataset...")
    train_dataset = load_kto_dataset(
